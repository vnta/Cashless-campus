...
void loop()                     // run over and over again
{
  ...
  if (p==FINGERPRINT_OK) Serial.println("model created");
    ...
    if (p==FINGERPRINT_PACKETRECIEVEERR) Serial.println("Packet rec err");
  ...
  logFile = SD.open("C12352.txt", FILE_WRITE);
  if (logFile)
  {
    Serial.println("File opeing successful for writing");
  }
  else
  {
