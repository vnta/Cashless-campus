...
char menu[]="00.txt";
char SBill[]="00000.txt", SArr[]="C00000.txt";
String SName;
...
Serial.println("yahhaaan");
  uint8_t p;
  File charIDs;
  byte Buf[80];
  int testRoll=00000, b, i;
  Serial.println(SRoll);
  charIDs=SD.open(SArr);
  if(charIDs){
    Serial.println("char id file open");
  }
  else {Serial.println("nah not open"); e=1; return;}
...
delay(100);
  ...
  else {Serial.println("not ready for pkt"); e=1; return;}
  ...
  while(b==0){
    ...
    else {Serial.println("some err in img2tz "); break;}
